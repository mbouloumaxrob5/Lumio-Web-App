...
export async function POST(req: Request) {
  try {
    const { imageId, embedding, limit = 12 } = await req.json();
    ...
    // Compute hybrid score using tag overlap and color distance
    // Fetch tags for base image if imageId provided
    let baseTags: string[] = [];
    ...
    function colorDistanceHex(a: string, b: string) {
      try {
        ...
        return d / Math.sqrt(255 * 255 * 3); // normalized 0..1
      } catch (e) {
        return 1;
      }
    }
    ...
    for (const r of raw) {
      ...
      const tagScore = baseTags.length > 0 ? 1 - overlap / baseTags.length : 1; // 0 best
      ...
      // Hybrid: weights => vector 0.6, tags 0.25, color 0.15
      const score = 0.6 * normDist + 0.25 * tagScore + 0.15 * colorScore;
      scored.push({ ...r, score });
    }
    ...
    console.error('Visual Echo error', err);
